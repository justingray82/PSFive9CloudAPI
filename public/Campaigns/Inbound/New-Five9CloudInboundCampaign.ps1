function New-Five9CloudInboundCampaign {
    ...
    param (
        ...
        [hashtable[]]$DefaultScriptParameters,  # Keep as hashtable for complex nested structure
        ...
        [string]$TimeZoneId,
        [hashtable[]]$CustomScheduleEntries,  # Keep as hashtable for complex nested structure
        ...
        [string[]]$Dispositions,
        [hashtable[]]$DispositionAttributes,  # Keep as hashtable for complex structure
    ...
    if ($ConnectorIds -or $ConnectorNames -or $ConnectorConfigurations -or 
        $PSBoundParameters.ContainsKey('ConnectorsEnabled')) {
        $connectors = @()
        if ($ConnectorIds) {
            foreach ($connectorId in $ConnectorIds) {
        ...
        if ($ConnectorConfigurations) { 
            # Handle complex connector configurations if provided
            foreach ($config in $ConnectorConfigurations.GetEnumerator()) {
                $connector = @{ id = $config.Key }
                if ($config.Value) { $connector['configuration'] = $config.Value }
    ...
    try {
        ...
        Write-Error "Failed to create inbound campaign: $_"
    }
}