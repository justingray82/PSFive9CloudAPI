function Remove-Five9CloudCampaignSurvey {
    ...
    $campaignDetails = Get-Five9CloudCampaignDetails -CampaignId $campaignId
    ...
    if ($result -ne $false) { Write-Host "$Survey removed on $CampaignName"} else { Write-Host "Unable to remove survey for $CampaignName"; return $false }
}
