function Set-Five9CloudCampaignWhisper {
    ...
    if ($result -ne $false) { Write-Host "$PromptName set as $CampaignName Whisper Prompt"} else { Write-Host "Failed to set $PromptName as $CampaignName Whisper Prompt"; return $false }
}