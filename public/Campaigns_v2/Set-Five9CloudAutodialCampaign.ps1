function Set-Five9CloudAutodialCampaign {
    param(
        [string]$CampaignId,
        [string]$CampaignName,
    ...
    if ($result -ne $false) { Write-Host "Autodial campaign '$CampaignName' updated successfully."; return $result } else { Write-Host "Failed to update autodial campaign '$CampaignName'."; return $false }
}
