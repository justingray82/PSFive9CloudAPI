function Set-Five9CloudUserPermission {
    param([string]$UserUID, [string]$Username, [string]$Permission, [switch]$Remove)
    if (-not $UserUID) { $UserUID = Resolve-Five9CloudUserUID $Username } ; if (-not $UserUID) { return }
    if ($Remove) {
        $result = Invoke-Five9CloudApi "$($global:Five9.ApiBaseUrl)/acl/v1/domains/$($global:Five9.DomainId)/users/$userUID/permissions/$Permission" -Method Delete
